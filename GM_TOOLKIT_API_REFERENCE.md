...
| Combat / encounter state | `/api/combat/state`, `/api/combat/{encounter_id}/...` | `CombatEncounterApiController`, `CombatApiController` | **Legacy/admin-only** (debug/testing/support). For player-facing encounter flow and gameplay mutations, use the coordinator (`/api/game/{campaign_id}/action`). |
