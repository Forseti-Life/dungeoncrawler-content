...
Ensures releases meet preconditions before state transitions.
"""
...
class ReleasePrerequisiteValidator:
    ...
    def get_active_teams(self) -> Dict[str, Dict[str, str]]:
        """Load active release-managed teams from product-teams.json.
    ...
    def validate_release_can_advance_to_push(
        ...
        """Validate that a release has its own prerequisites for push.

        Each product team now ships independently. A release may advance toward
        push as soon as the owning PM has signed off.
