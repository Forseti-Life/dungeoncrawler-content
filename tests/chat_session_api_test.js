...
(async () => {
  ...
  // --- requestLocationGeneration ---
  resetFetch();
  mockFetch({ success: true, data: { room_id: 'room_new', navigation: { target_room_id: 'room_new' } } });
  await api.requestLocationGeneration({ destination: 'blacksmith cellar', origin_room_id: 'room_a1', party_level: 3 });
  assert(fetchCalls[0].url === '/api/campaign/42/gm/locations/request', 'requestLocationGeneration URL');
  const locationBody = JSON.parse(fetchCalls[0].opts.body);
  assert(locationBody.destination === 'blacksmith cellar', 'requestLocationGeneration destination');
  assert(locationBody.origin_room_id === 'room_a1', 'requestLocationGeneration origin room');
  assert(locationBody.party_level === 3, 'requestLocationGeneration party level');

  // --- requestRoomGeneration ---
  resetFetch();
  mockFetch({ success: true, data: { room_id: 'room_side', navigation: { target_room_id: 'room_side' } } });
  await api.requestRoomGeneration({ origin_room_id: 'room_a1', level_id: 'lvl_1', room_type: 'library' });
  assert(fetchCalls[0].url === '/api/campaign/42/gm/rooms/request', 'requestRoomGeneration URL');
  const roomBody = JSON.parse(fetchCalls[0].opts.body);
  assert(roomBody.origin_room_id === 'room_a1', 'requestRoomGeneration origin room');
  assert(roomBody.level_id === 'lvl_1', 'requestRoomGeneration level');
  assert(roomBody.room_type === 'library', 'requestRoomGeneration room type');

  // --- requestLocationQuests ---
  resetFetch();
  mockFetch({ success: true, data: { quests: [] } });
  await api.requestLocationQuests({ room_id: 'room_side', count: 2 });
  assert(fetchCalls[0].url === '/api/campaign/42/gm/quests/request', 'requestLocationQuests URL');
  const questBody = JSON.parse(fetchCalls[0].opts.body);
  assert(questBody.room_id === 'room_side', 'requestLocationQuests room id');
  assert(questBody.count === 2, 'requestLocationQuests count');

  // --- generateDungeon ---
  resetFetch();
  mockFetch({ dungeon_id: 'dgn_1', name: 'The Goblin Warren' });
  await api.generateDungeon({ location_x: 10, location_y: 20, party_level: 4, theme: 'goblin_warrens' });
  assert(fetchCalls[0].url === '/api/campaign/42/dungeons/generate', 'generateDungeon URL');
  const dungeonBody = JSON.parse(fetchCalls[0].opts.body);
  assert(dungeonBody.location_x === 10, 'generateDungeon location_x');
  assert(dungeonBody.location_y === 20, 'generateDungeon location_y');
  assert(dungeonBody.party_level === 4, 'generateDungeon party level');
  assert(dungeonBody.theme === 'goblin_warrens', 'generateDungeon theme');
