...
class NarrationEngineWiringTest extends UnitTestCase {
  ...
  /**
   * Tests narrator speech-like content is remapped to GM scope.
   */
  public function testNormalizeEventRoleScopeReassignsNarratorSpeechToGm(): void {
    $normalized = $this->invokeNormalizeEventRoleScope([
      'type' => 'movement',
      'speaker' => 'Narrator',
      'speaker_type' => 'narrator',
      'content' => 'Narrator says "Stay sharp."',
    ]);

    $this->assertSame('Game Master', $normalized['speaker']);
    $this->assertSame('gm', $normalized['speaker_type']);
  }

  /**
   * Tests narrator-owned event types remain narrator-scoped.
   */
  public function testNormalizeEventRoleScopeKeepsNarratorOwnershipForNarratorEvents(): void {
    $normalized = $this->invokeNormalizeEventRoleScope([
      'type' => 'round_start',
      'speaker' => 'Narrator',
      'speaker_type' => '',
    ...
    $this->assertSame('Narrator', $normalized['speaker']);
    $this->assertSame('narrator', $normalized['speaker_type']);
  }

  /**
   * Invoke protected normalizeEventRoleScope with a real service instance.
   */
  private function invokeNormalizeEventRoleScope(array $event): array {
    $engine = $this->createEngine();
    $method = new \ReflectionMethod(NarrationEngine::class, 'normalizeEventRoleScope');
    $method->setAccessible(TRUE);
    return $method->invoke($engine, $event);
  }

  /**
   * Build NarrationEngine with mocked constructor dependencies.
   */
  private function createEngine(): NarrationEngine {
    $logger = $this->createMock(LoggerInterface::class);
    $loggerFactory = $this->createMock(LoggerChannelFactoryInterface::class);
    $loggerFactory->method('get')
      ->with('dungeoncrawler_narration')
