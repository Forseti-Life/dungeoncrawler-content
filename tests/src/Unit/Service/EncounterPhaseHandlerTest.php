...
class EncounterPhaseHandlerTest extends UnitTestCase {
  ...
  /**
   * De-escalation intent should produce a deterministic multi-action plan.
   *
   * @covers ::buildNpcTurnPlan
   */
  public function testBuildNpcTurnPlanMaintainsDeescalationContinuity(): void {
    $psychology = $this->createMock(NpcPsychologyService::class);
    $psychology->method('loadProfile')->willReturnMap([
      [42, 'npc_friendly', [
        'display_name' => 'Friendly Scout',
    ...
    $this->assertSame('deescalate', $plan['intent_contract']['intent']);
    $this->assertSame(['talk', 'stride', 'end_turn'], array_map(static fn(array $step): string => (string) ($step['type'] ?? ''), $steps));
    $this->assertStringContainsString('de-escalation', (string) ($steps[0]['decision_reason'] ?? ''));
    $this->assertSame('deescalate', $steps[0]['decision_basis']['intent'] ?? NULL);
    $this->assertSame('deescalate', $steps[1]['decision_basis']['intent'] ?? NULL);
    $this->assertSame('deescalate', $steps[2]['decision_basis']['intent'] ?? NULL);
  }

  /**
   * Self-preservation intent should hold across all planned actions.
   *
   * @covers ::buildNpcTurnPlan
   */
  public function testBuildNpcTurnPlanMaintainsSelfPreservationContinuity(): void {
    $psychology = $this->createMock(NpcPsychologyService::class);
    $psychology->method('loadProfile')->willReturnMap([
      [77, 'npc_coward', [
        'display_name' => 'Shaken Raider',
    ...
    $steps = $plan['steps'] ?? [];
    $this->assertCount(3, $steps);
    $this->assertSame('self_preserve', $plan['intent_contract']['intent']);
    $this->assertSame(['stride', 'stride', 'end_turn'], array_map(static fn(array $step): string => (string) ($step['type'] ?? ''), $steps));
    $this->assertSame('self_preserve', $steps[0]['decision_basis']['intent'] ?? NULL);
    $this->assertSame('self_preserve', $steps[1]['decision_basis']['intent'] ?? NULL);
    $this->assertSame('self_preserve', $steps[2]['decision_basis']['intent'] ?? NULL);
  }

  /**
   * High-cunning intent should keep weakest-target pressure across the turn.
   *
   * @covers ::buildNpcTurnPlan
   */
  public function testBuildNpcTurnPlanMaintainsCunningWeakTargetContinuity(): void {
    $psychology = $this->createMock(NpcPsychologyService::class);
    $psychology->method('loadProfile')->willReturnMap([
      [13, 'npc_hunter', [
        'display_name' => 'Hunter',
    ...
    $steps = $plan['steps'] ?? [];
    $this->assertCount(3, $steps);
    $this->assertSame('finish_weakest', $plan['intent_contract']['intent']);
    $this->assertSame(['strike', 'strike', 'strike'], array_map(static fn(array $step): string => (string) ($step['type'] ?? ''), $steps));
    $this->assertSame('pc-weak', $steps[0]['target_instance_id'] ?? NULL);
    $this->assertSame('pc-weak', $steps[1]['target_instance_id'] ?? NULL);
    $this->assertSame('pc-weak', $steps[2]['target_instance_id'] ?? NULL);
  }

  /**
   * Room-scene pass events should carry decision telemetry.
   *
   * @covers ::passRoomActorTurn
   */
  public function testPassRoomActorTurnIncludesDecisionTelemetry(): void {
    $handler = $this->buildHandler();
    $game_state = [
      'campaign_id' => 1,
      'round' => 3,
      'turn' => ['actions_remaining' => 2],
      'initiative_order' => [
        ['entity_id' => 'npc-1', 'team' => 'enemy', 'position_q' => 0, 'position_r' => 0],
      ],
      'encounter_context' => ['room_id' => 'room-a'],
    ];
    $dungeon_data = [
    ...
    $result = $this->invokePassRoomActorTurn($handler, 'npc-1', $game_state, $dungeon_data, 1);
    $event_data = $result['events'][0]['data'] ?? [];

    $this->assertArrayHasKey('decision_reason', $event_data);
    $this->assertArrayHasKey('decision_basis', $event_data);
    $this->assertSame('room_scene', $event_data['decision_basis']['mode'] ?? NULL);
  }
  ...
  /**
   * Invoke protected NPC multi-action turn planner.
   *
  ...
  /**
   * Invoke protected room-scene pass action with by-reference state.
   *
   * @return array<string, mixed>
   *   Pass-turn result payload.
   */
  private function invokePassRoomActorTurn(
    EncounterPhaseHandler $handler,
    string $entity_id,
    array &$game_state,
    array &$dungeon_data,
    ...
    $method = new \ReflectionMethod(EncounterPhaseHandler::class, 'passRoomActorTurn');
    $method->setAccessible(TRUE);
    $args = [$entity_id, &$game_state, &$dungeon_data, $campaign_id];
