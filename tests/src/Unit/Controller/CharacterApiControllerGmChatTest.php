...
class CharacterApiControllerGmChatTest extends TestCase {
  ...
  /**
   * Tests validation errors preserve their client status code.
   */
  public function testGmChatReturnsClientValidationStatus(): void {
    $character_manager = $this->createMock(CharacterManager::class);
    $gm_service = $this->createMock(CharacterCreationGmService::class);
    $gm_service->expects($this->once())
      ->method('handleMessage')
      ->willThrowException(new \InvalidArgumentException('GM faction creation update is incomplete.', 400));
    ...
    $logger_factory->method('get')->with('dungeoncrawler_content')->willReturn($logger);
    ...
    $request = new Request([], [], [], [], [], [], json_encode([
      'step' => 1,
      'message' => 'Create a new dockside faction.',
    ]));
    $request->headers->set('X-CSRF-Token', 'valid-token');
    ...
    $this->assertSame(400, $response->getStatusCode());
    $this->assertFalse($payload['success'] ?? TRUE);
    $this->assertSame('GM faction creation update is incomplete.', $payload['error'] ?? '');
  }
