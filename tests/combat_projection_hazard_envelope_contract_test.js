...
assert(
  chatPanelSource.includes("const damagePacket = findResolutionPacket('damage_application')")
    ...
    && chatPanelSource.includes('const resolvedDamage = Number(damagePacket?.amount);')
    && chatPanelSource.includes('damagePacket?.damage_type')
    && !chatPanelSource.includes('hazard_resolution_envelope'),
  'Hazard projection consumes only the action resolution-envelope damage packet'
);
