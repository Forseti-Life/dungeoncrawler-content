...
- **Admin settings**: Configure under `/admin/config/content/dungeoncrawler`:
   ...
   - Vertex live settings + service-account credentials (preferred via `GOOGLE_APPLICATION_CREDENTIALS`, `VERTEX_SERVICE_ACCOUNT_FILE`, `VERTEX_SERVICE_ACCOUNT_JSON`, or config-backed service-account fields)
- **Vertex auth note**: Vertex image generation now uses OAuth service-account credentials rather than API-key authentication.
