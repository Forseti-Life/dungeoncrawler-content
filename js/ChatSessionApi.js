 ...
 *   POST /api/campaign/{cid}/gm/locations/request       — GM-only location request
 *   POST /api/campaign/{cid}/gm/rooms/request           — GM-only procedural room request
 *   POST /api/campaign/{cid}/gm/quests/request          — GM-only location quest request
 *   POST /api/campaign/{cid}/dungeons/generate          — GM-only dungeon generation
...
export class ChatSessionApi {
  ...
  /**
   * Request a new GM-generated location from the current room.
   * @param {object} payload
   * @param {string} payload.destination
   * @param {string} payload.origin_room_id
   * @param {number} [payload.party_level]
   * @returns {Promise<object>}
   */
  async requestLocationGeneration(payload) {
    const result = await postJson(`${this.baseUrl}/gm/locations/request`, payload);
    if (!result.success) throw new Error(result.error || 'Location request failed');
    return result.data;
  }

  /**
   * Request a new procedural room in the current dungeon.
   * @param {object} payload
   * @returns {Promise<object>}
   */
  async requestRoomGeneration(payload) {
    const result = await postJson(`${this.baseUrl}/gm/rooms/request`, payload);
    if (!result.success) throw new Error(result.error || 'Room request failed');
    return result.data;
  }

  /**
   * Request location-scoped quests for the current room.
   * @param {object} payload
   * @returns {Promise<object>}
   */
  async requestLocationQuests(payload) {
    const result = await postJson(`${this.baseUrl}/gm/quests/request`, payload);
    if (!result.success) throw new Error(result.error || 'Quest request failed');
    return result.data;
  }

  /**
   * Generate a new dungeon site for the campaign.
   * @param {object} payload
   * @returns {Promise<object>}
   */
  async generateDungeon(payload) {
    return postJson(`${this.baseUrl}/dungeons/generate`, payload);
  }
