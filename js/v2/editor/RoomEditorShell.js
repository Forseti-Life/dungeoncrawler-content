...
import { GameEventBus } from '../GameEventBus.js?v=20260908g1';
import { HexCanvas } from '../canvas/HexCanvas.js?v=20260908g1';
import { SpriteService } from '../../SpriteService.js?v=20260908g1';
...
export class RoomEditorShell {
  ...
  _bindDom() {
    ...
    this._dom = {
      ...
      generatePrompt: q('[data-room-editor-generation-prompt]'),
      generateTheme: q('[data-room-editor-generation-theme]'),
      generateSize: q('[data-room-editor-generation-size]'),
      generateMinHexes: q('[data-room-editor-generation-min-hexes]'),
      generateMaxHexes: q('[data-room-editor-generation-max-hexes]'),
      generateSeed: q('[data-room-editor-generation-seed]'),
      generateBtn: q('[data-room-editor-action="generate-room-layout"]'),
  ...
  _buildRoomInspector() {
    ...
    if (room.metadata?.generated_by) {
      const provenance = document.createElement('p');
      provenance.className = 'room-editor__hint';
      provenance.textContent = `Generated by ${room.metadata.generated_by.model || 'unknown model'} with seed ${room.metadata.generated_by.seed ?? '?'}.`;
      wrapper.appendChild(provenance);
    }
  ...
  _bindGmEvents() {
    ...
    this._dom.generateBtn?.addEventListener('click', () => this._generateRoomLayout());
  ...
  async _generateRoomLayout() {
    const prompt = String(this._dom.generatePrompt?.value || '').trim();
    if (!prompt) {
      this._setStatus('Enter a generation prompt first.', 'error');
      return null;
    }
    const args = {
      prompt,
      theme: String(this._dom.generateTheme?.value || '').trim(),
      size_category: String(this._dom.generateSize?.value || 'medium'),
      min_hexes: Number.parseInt(this._dom.generateMinHexes?.value || '16', 10),
      max_hexes: Number.parseInt(this._dom.generateMaxHexes?.value || '32', 10),
    };
    const seed = String(this._dom.generateSeed?.value || '').trim();
    if (seed !== '') {
      args.seed = Number.parseInt(seed, 10);
    }
    this._waitingOnGenerator = true;
    try {
      return await this._invokeGmTool('generate_room_layout', args, false);
    } finally {
      this._waitingOnGenerator = false;
      this._setBusy(false);
    }
  ...
  async _sendGmRequest(intent, dryRun, label) {
    ...
    return this._runExclusive(async () => {
      this._setGmState(toolName === 'generate_room_layout' ? 'Waiting on generator (LLM)…' : `Running ${toolName}...`);
      let envelope = null;
      try {
        const result = await this._postJson(url, body);
  ...
  _setBusy(busy) {
    const controls = [
      ...
      this._dom.validateBtn, this._dom.roomSelect, this._dom.generateBtn,
    ];
    controls.forEach((el) => {
      if (el) {
        el.disabled = busy;
    ...
    if (this._dom.generateBtn) {
      this._dom.generateBtn.textContent = busy && this._waitingOnGenerator ? 'Waiting on generator (LLM)…' : 'Generate room plan';
    }
    if (!busy) {
      this._updateHistoryButtons();
    }
