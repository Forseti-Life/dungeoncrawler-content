...
import './placementTransform.js?v=20260908g1';
...
export class DungeonEditorShell {
  ...
  _bindDom() {
    ...
    this._dom = {
      ...
      generatePrompt: q('[data-dungeon-editor-generation-prompt]'),
      generateTheme: q('[data-dungeon-editor-generation-theme]'),
      generateRoomCount: q('[data-dungeon-editor-generation-room-count]'),
      generateSeed: q('[data-dungeon-editor-generation-seed]'),
      generateBtn: q('[data-dungeon-editor-action="generate-dungeon-layout"]'),
  ...
  _bindGmEvents() {
    ...
    this._dom.generateBtn?.addEventListener('click', () => this._generateDungeonLayout());
  ...
  _renderInspector() {
    ...
    if (!placement) {
      const generatedBy = this.model?.metadata?.generated_by;
      if (generatedBy) {
        body.appendChild(makeEl('p', 'room-editor__hint', `Generated by ${generatedBy.model || 'unknown model'} with seed ${generatedBy.seed ?? '?'}.`));
      }
  ...
  async _generateDungeonLayout() {
    ...
    const args = {
      ...
      room_count: Number.parseInt(this._dom.generateRoomCount?.value || '3', 10),
    };
    ...
    try {
      return await this._invokeGmTool('generate_dungeon_layout', args, false);
    } finally {
      this._waitingOnGenerator = false;
      this._setBusy(false);
    }
  ...
  async _sendGmRequest(intent, dryRun, label) {
    ...
    this._setBusy(true);
    this._setGmState(label === 'generate_dungeon_layout' ? 'Waiting on generator (LLM)…' : `Running ${label}...`);
    let envelope = null;
    try {
  ...
  _setBusy(busy) {
    this._busy = busy;
    [this._dom.loadBtn, this._dom.newBtn, this._dom.dungeonSelect, this._dom.publishButton, this._dom.generateBtn].forEach((el) => {
      if (el) {
        el.disabled = busy;
      }
    });
    if (this._dom.generateBtn) {
      this._dom.generateBtn.textContent = busy && this._waitingOnGenerator ? 'Waiting on generator (LLM)…' : 'Generate dungeon plan';
    }
    this._updateHistoryButtons();
    this._renderPublicationReadiness();
  }
