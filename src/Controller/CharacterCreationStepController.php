...
class CharacterCreationStepController extends ControllerBase {
  ...
  private function buildPortraitSummary(array $result): array {
    ...
    if (!empty($result['provider_status']) && is_array($result['provider_status'])) {
      $summary['provider_status'] = [
        ...
        'has_api_key' => (bool) (($result['provider_status']['has_credentials'] ?? FALSE) || ($result['provider_status']['has_api_key'] ?? FALSE)),
      ];
    }
  ...
  private function updateStepData(array $character_data, int $step, array $form_data) {
    ...
    if (in_array($step, [2, 3, 4, 5], TRUE)) {
      $tracker = new AbilityScoreTracker($this->characterManager);
      $character_data = $this->storeCalculatedAbilityScores(
        $character_data,
        $tracker->calculateAbilityScores($character_data)
      );
    }
