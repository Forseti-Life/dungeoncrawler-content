 ...
 */
class ExplorationPhaseHandler implements MutationContextPhaseHandlerInterface {
    ...
    foreach ($changed_slices as $slice_key) {
      ...
      if ($slice_key === 'actor_entities') {
        if ($targets['entity_ids'] === []) {
          throw new \RuntimeException('Exploration mutation envelope contract violation: actor_entities changed without entity mutation targets.');
        }
      ...
      elseif ($slice_key === 'rooms') {
        if ($targets['room_ids'] === []) {
          throw new \RuntimeException('Exploration mutation envelope contract violation: rooms changed without room mutation targets.');
        }
      ...
      elseif ($slice_key === 'connections') {
        if ($targets['connection_ids'] === [] && $targets['room_ids'] === []) {
          throw new \RuntimeException('Exploration mutation envelope contract violation: connections changed without connection/room mutation targets.');
        }
  ...
  protected function computeRuntimeSliceFingerprints(array $dungeon_data): array {
    ...
    $encode = static function (array $payload): string {
      ...
      if (!is_string($encoded)) {
        throw new \RuntimeException('Exploration runtime-slice fingerprint contract violation: unable to encode payload.');
      }
