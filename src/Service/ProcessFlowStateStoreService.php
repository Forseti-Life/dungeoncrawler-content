...
/**
 * Persistence scaffold for actor process-flow snapshots.
 */
class ProcessFlowStateStoreService {
  ...
  /**
   * Persist latest process-flow summary for one actor.
   *
  ...
  public function storeLatestState(int $campaign_id, string $entity_ref, array $summary, array $meta = []): array {
    ...
    $state['process_flow_state'] = $registry;
  ...
  /**
   * Load latest process-flow summary for one actor when available.
   */
  ...
  /**
   * Persist latest process-flow snapshot to canonical table when available.
   *
  ...
  protected function persistStateRow(int $campaign_id, array $snapshot): void {
    ...
    $this->database->merge('dc_process_flow_state')
      ->keys([
  ...
  /**
   * Build canonical entity-ref candidate keys for state lookup.
   *
