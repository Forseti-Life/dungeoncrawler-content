...
class DungeonGeneratorService {
  ...
  public function generateDungeon(array $context): array {
    $this->assertR5Enabled('/canonical_runtime_generation/r5');
  ...
  private function assertR5Enabled(string $pointer): void {
    if (!$this->configFactory || $this->configFactory->get('dungeoncrawler_content.settings')->get('canonical_runtime_generation.r5') === FALSE) {
      throw new RuntimeGenerationException('runtime_generation_failed', [[
        'code' => 'runtime_generation_failed',
        'pointer' => $pointer,
        'message' => 'R5 canonical dungeon generation is disabled; no legacy fallback is available.',
        'severity' => 'error',
      ]], 503);
    }
  }
