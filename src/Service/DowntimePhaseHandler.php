...
class DowntimePhaseHandler implements PhaseHandlerInterface {
  ...
  /**
   * Build typed mutation envelope from downtime runtime context.
   *
  ...
  protected function processLongRest(?string $actor_id, array $params, array &$game_state, array &$dungeon_data, int $campaign_id): array {
    ...
    if ($actor_id && !empty($dungeon_data['entities'])) {
      foreach ($dungeon_data['entities'] as &$entity) {
        ...
        if ($iid === $actor_id) {
          $campaign_character_id = (string) ($entity['character_id'] ?? $entity['state']['characterId'] ?? '');
          $canonical_state = NULL;
          if ($campaign_character_id !== '') {
            try {
              $canonical_state = $this->characterStateService->getState($campaign_character_id, $campaign_id, (string) $iid);
            }
            catch (\Throwable $e) {
              $this->logger->warning('Unable to load canonical character state for long rest: @error', ['@error' => $e->getMessage()]);
            }
          }

          $current_hp = (int) ($canonical_state['resources']['hitPoints']['current']
            ?? $entity['state']['hit_points']['current']
            ?? 0);
          $max_hp = (int) ($canonical_state['resources']['hitPoints']['max']
            ?? $entity['state']['hit_points']['max']
            ?? 20);
          ...
          $con_mod = $canonical_state !== NULL
            ? (int) floor(($constitution - 10) / 2)
            : (int) ($entity['stats']['con_modifier'] ?? 0);
          $level = max(1, (int) ($canonical_state['basicInfo']['level'] ?? $entity['stats']['level'] ?? 1));
          ...
          $heritage = (string) ($canonical_state['basicInfo']['heritage'] ?? $entity['heritage'] ?? '');
          if ($heritage === 'hillock') {
            $hp_per_rest += $level;
          }
          if (!$healing_blocked) {
          ...
          if ($canonical_state !== NULL) {
            $canonical_state['resources']['hitPoints']['current'] = $new_hp;
          }
          ...
          if ($canonical_state !== NULL) {
            $canonical_state['conditions'] = array_values(array_filter(
              is_array($canonical_state['conditions'] ?? NULL) ? $canonical_state['conditions'] : [],
              function ($condition) use (&$conditions_removed) {
                $name = $condition['name'] ?? ($condition['type'] ?? '');
                if ($name === 'wounded') {
                  $conditions_removed[] = 'wounded';
          ...
          if ($canonical_state !== NULL) {
            $canonical_state['hours_since_rest'] = 0;
          }
          ...
          if ($canonical_state !== NULL) {
            foreach ($canonical_state['conditions'] as &$cond) {
              $cname = $cond['name'] ?? ($cond['type'] ?? '');
              if ($cname === 'doomed') {
                $cond['value'] = max(0, (int) ($cond['value'] ?? 1) - 1);
                if ($cond['value'] <= 0) {
            ...
            if (is_array($canonical_state['resources']['spellSlots'] ?? NULL)) {
              foreach ($canonical_state['resources']['spellSlots'] as $slot_key => $slot_state) {
                $canonical_state['resources']['spellSlots'][$slot_key]['current'] = (int) ($slot_state['max'] ?? 0);
              }
            }
            if (is_array($canonical_state['resources']['focusPoints'] ?? NULL)) {
              $canonical_state['resources']['focusPoints']['current'] = (int) ($canonical_state['resources']['focusPoints']['max'] ?? 0);
            }
