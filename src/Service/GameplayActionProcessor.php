...
PARTYRULES;
    ...
    $storage_owner_details = $room_inventory_data['storage_owner_details'] ?? [];
    if (!empty($storage_owner_details)) {
      $enhanced .= "\nAuthoritative storage inventories:\n";
      foreach ($storage_owner_details as $owner_detail) {
        $owner_name = $owner_detail['name'] ?? ($owner_detail['owner_type'] ?? 'storage');
        $owner_type = $owner_detail['owner_type'] ?? 'unknown';
        $owner_id = $owner_detail['owner_id'] ?? '';
        if ($owner_id === '') {
          continue;
        }

        $owner_currency = $owner_detail['currency'] ?? ['cp' => 0, 'sp' => 0, 'gp' => 0, 'pp' => 0];
        $owner_currency_parts = [];
        foreach (['pp', 'gp', 'sp', 'cp'] as $denomination) {
          $amount = (int) ($owner_currency[$denomination] ?? 0);
          if ($amount > 0) {
            $owner_currency_parts[] = "{$amount} {$denomination}";
          }
        }
        ...
        foreach (($owner_detail['items'] ?? []) as $item) {
          ...
          $item_suffix = $item_id !== '' ? " {id: {$item_id}}" : '';
          $owner_items[] = $item_quantity > 1
            ? "{$item_name} (x{$item_quantity}){$item_suffix}"
            : "{$item_name}{$item_suffix}";
        }

        $enhanced .= "  - {$owner_name} ({$owner_type}) {owner_id: {$owner_id}}"
          . " | Currency: " . (!empty($owner_currency_parts) ? implode(', ', $owner_currency_parts) : '0 gp')
          . " | Items: " . (!empty($owner_items) ? implode(', ', $owner_items) : 'None')
          . "\n";
      }
    }
...
Currency transfer format:
```json
{
  "actions": [
    {
      "type": "transfer_currency",
      "name": "Pay Eldric 2 gp",
      "details": {
        "currency_transfer": {
          "source_owner_type": "character",
          "source_owner_id": "ACTING_CHARACTER",
          "dest_owner_type": "character",
          "dest_owner_id": "target owner id",
          "amount": 2,
          "denomination": "gp"
        },
        "result_description": "Brief payment or coin transfer outcome"
      },
      "state_changes": { "character": {}, "room": {} }
    }
  ]
...
Inventory consumption format:
```json
{
  "actions": [
    {
      "type": "consume_inventory",
      "name": "Drink the healing potion",
      "details": {
        "consume": {
          "source_owner_type": "character",
          "source_owner_id": "ACTING_CHARACTER",
        ...
        "result_description": "Brief consumption outcome"
      },
      "state_changes": {
        "character": {
          "hp_delta": 0,
...
8. Before narrating a payment, gift, trade, use, or consumption of an item, verify the acting character or other storage owner actually has that currency or item in the authoritative inventory/currency context above.
9. Use currency_delta only for character-only currency changes that do not move coins between owners. For payments, gifts, tips, or receiving coins from another owner, use transfer_currency.
...
13. For paying or receiving coins between owners, use the action type "transfer_currency" with exact owner ids and denomination.
14. For drinking, eating, applying, lighting, or otherwise consuming a specific carried item, use the action type "consume_inventory" with the exact item_instance_id.
15. A transfer_inventory or consume_inventory action must reference the exact item_instance_id from the authoritative prompt context.
16. Do not invent storage owner ids, item instance ids, or container ids. If the source/destination cannot be identified exactly, narrate uncertainty instead of claiming a completed transfer or consumption.
17. For delivering or turning in a quest item/objective, use "quest_turn_in" with the quest payload instead of only describing it narratively.
18. For starting a fight, use "combat_initiation" instead of only narrating that combat begins.
...
ENTRY_NARRATION_RULES;
  ...
  public function applyCharacterStateChanges(int $character_id, array $actions, ?int $campaign_id = NULL): array {
    ...
    foreach ($actions as $action) {
      ...
      if (($action['type'] ?? '') === 'transfer_currency') {
        $transfer_result = $this->applyTransferCurrencyAction($action, $character_id, $campaign_id);
      ...
      if (($action['type'] ?? '') === 'consume_inventory') {
        $consume_result = $this->applyConsumeInventoryAction($action, $character_id, $campaign_id);
        if (!empty($consume_result['applied'])) {
          if (!empty($consume_result['removed_label'])) {
            $diff['inventory_removed'][] = $consume_result['removed_label'];
          }
          $changed = TRUE;
        }
      }
  ...
  public function validateCharacterActionResources(int $character_id, array $actions, ?int $campaign_id = NULL): array {
    ...
    foreach ($actions as $action) {
      ...
      if (($action['type'] ?? '') === 'transfer_currency') {
        $currency_validation = $this->validateTransferCurrencyAction($action, $character_id, $campaign_id);
        if (empty($currency_validation['valid'])) {
          $action_errors = array_merge($action_errors, $currency_validation['errors'] ?? []);
        }
      ...
      if (($action['type'] ?? '') === 'consume_inventory') {
        $consume_validation = $this->validateConsumeInventoryAction($action, $character_id, $campaign_id);
        if (empty($consume_validation['valid'])) {
          $action_errors = array_merge($action_errors, $consume_validation['errors'] ?? []);
        }
        if (!empty($action_errors)) {
          $errors[] = [
            'action_name' => $action_name,
  ...
  /**
   * Flatten an inventory payload returned by InventoryManagementService.
   */
  protected function flattenStructuredInventoryItems(array $inventory): array {
    if (!is_array($inventory)) {
      return [];
    }
    ...
    if (isset($inventory['items']) && is_array($inventory['items'])) {
      return array_values(array_filter(array_map(static function (array $item): array {
        ...
        return [
          ...
          'name' => $state['name'] ?? ($item['item_id'] ?? 'Unknown item'),
          'quantity' => max(1, (int) ($item['quantity'] ?? 1)),
        ];
      }, $inventory['items']), 'is_array'));
    }

    if (isset($inventory['carried']) || isset($inventory['worn']) || isset($inventory['equipped']) || isset($inventory['stashed'])) {
      return $this->extractInventoryItems(['inventory' => $inventory]);
    }
  ...
  /**
   * Validate a centralized currency transfer action.
   */
  protected function validateTransferCurrencyAction(array $action, int $acting_character_id, ?int $campaign_id = NULL): array {
      $transfer = $this->extractCurrencyTransferSpec($action, $acting_character_id);
      if (empty($transfer['valid'])) {
        return $transfer;
      }

      $validation = $this->inventoryManagementService->validateTransferCurrencyTransaction(
        $transfer['source'],
        $transfer['destination'],
      ...
      if (empty($validation['valid'])) {
        return [
          'valid' => FALSE,
          'errors' => $validation['errors'] ?? ['Currency transfer could not be validated.'],
        ];
      }
  ...
  /**
   * Validate a centralized inventory consumption action.
   */
  protected function validateConsumeInventoryAction(array $action, int $acting_character_id, ?int $campaign_id = NULL): array {
      $consume = $this->extractConsumeSpec($action, $acting_character_id);
      if (empty($consume['valid'])) {
        return $consume;
      }

      $validation = $this->inventoryManagementService->validateConsumeItemTransaction(
        $consume['source'],
        $consume['item_instance_id'],
        $consume['quantity'],
        $campaign_id
      );
      ...
      if (empty($validation['valid'])) {
        return [
          'valid' => FALSE,
          'errors' => $validation['errors'] ?? ['Inventory consumption could not be validated.'],
        ];
      }
  ...
  /**
   * Execute a centralized currency transfer action.
   */
  protected function applyTransferCurrencyAction(array $action, int $acting_character_id, ?int $campaign_id = NULL): array {
      $transfer = $this->extractCurrencyTransferSpec($action, $acting_character_id);
      if (empty($transfer['valid'])) {
        return ['applied' => FALSE];
      }

      $result = $this->inventoryManagementService->transferCurrencyTransaction(
        $transfer['source'],
        $transfer['destination'],
        $transfer['denomination'],
        $transfer['amount'],
      ...
      $label = $transfer['amount'] . ' ' . $transfer['denomination'];
  ...
  /**
   * Execute a centralized inventory consumption action.
   */
  protected function applyConsumeInventoryAction(array $action, int $acting_character_id, ?int $campaign_id = NULL): array {
      $consume = $this->extractConsumeSpec($action, $acting_character_id);
      if (empty($consume['valid'])) {
        return ['applied' => FALSE];
      }

      $result = $this->inventoryManagementService->consumeItemTransaction(
        $consume['source'],
        $consume['item_instance_id'],
        $consume['quantity'],
        $campaign_id
      ...
      $label = $result['item_name'] ?? 'Item';
      if (($consume['quantity'] ?? 1) > 1) {
        $label = $consume['quantity'] . 'x ' . $label;
      }

      return [
        'applied' => !empty($result['success']),
        'removed_label' => ((string) $consume['source']['owner_id'] === (string) $acting_character_id && $consume['source']['owner_type'] === 'character') ? $label : NULL,
      ];
  }
  ...
  /**
   * Extract and normalize a currency transfer spec from an action payload.
   */
  protected function extractCurrencyTransferSpec(array $action, int $acting_character_id): array {
      $transfer = $action['details']['currency_transfer'] ?? [];
      if (!is_array($transfer)) {
        return [
          'valid' => FALSE,
      ...
      if ($amount < 1) {
        $errors[] = 'missing or invalid currency amount';
      }
      if (!in_array($denomination, ['cp', 'sp', 'gp', 'pp'], TRUE)) {
        $errors[] = 'missing or invalid currency denomination';
      }
  ...
  /**
   * Extract and normalize the consume spec from an action payload.
   */
  protected function extractConsumeSpec(array $action, int $acting_character_id): array {
      $consume = $action['details']['consume'] ?? [];
      if (!is_array($consume)) {
        return [
          'valid' => FALSE,
      ...
      $item_instance_id = (string) ($consume['item_instance_id'] ?? '');
  ...
  public function buildRoomInventory(int $campaign_id, string $room_id, array $room_meta, array $dungeon_data): array {
    ...
    try {
      $entity_rows = $this->database->select('dc_campaign_characters', 'e')
        ->fields('e', ['id', 'name', 'type', 'state_data', 'character_data'])
        ->condition('campaign_id', $campaign_id)
      ...
      foreach ($entity_rows as $row) {
        ...
        $ename = $row->name ?? ($echar['name'] ?? 'Unknown');
        $etype = $row->type ?? 'npc';
        ...
        switch ($etype) {
          ...
          case 'creature':
            $inventory['npcs'][] = [
              ...
              'owner_id' => (string) ($echar['id'] ?? $estate['characterId'] ?? $row->id ?? ''),
            ];
            $entity_owner_id = (string) ($echar['id'] ?? $estate['characterId'] ?? $row->id ?? '');
            if ($entity_owner_id !== '') {
              $inventory['storage_owners'][] = [
    ...
    $npc_names_by_owner = [];
    foreach ($inventory['npcs'] as $npc) {
      $npc_owner_id = (string) ($npc['owner_id'] ?? '');
      $npc_name = (string) ($npc['name'] ?? '');
      if ($npc_owner_id !== '' && $npc_name !== '') {
        $npc_names_by_owner[$npc_owner_id] = $npc_name;
      }
    }
    foreach ($inventory['storage_owners'] as &$owner) {
      $owner_id = (string) ($owner['owner_id'] ?? '');
      if (($owner['name'] ?? '') === '' && isset($npc_names_by_owner[$owner_id])) {
        $owner['name'] = $npc_names_by_owner[$owner_id];
      }
    }
    unset($owner);
    ...
    foreach ($inventory['storage_owners'] as $owner) {
      ...
      try {
        ...
        if ($owner_name === '' && $owner_type === 'character') {
          $owner_name = (string) ($this->database->select('dc_campaign_characters', 'c')
            ->fields('c', ['name'])
        ...
        $owner_inventory = $this->inventoryManagementService->getInventory($owner_id, $owner_type, $campaign_id);
        $items = [];
        foreach ($this->flattenStructuredInventoryItems($owner_inventory) as $item) {
          $items[] = [
            'item_instance_id' => $item['item_instance_id'] ?? '',
            'name' => $item['name'] ?? ($item['item_id'] ?? 'Unknown item'),
            'quantity' => max(1, (int) ($item['quantity'] ?? 1)),
          ];
        }

        $inventory['storage_owner_details'][] = [
          'owner_id' => $owner_id,
          'owner_type' => $owner_type,
          'name' => $owner_name,
          'currency' => $owner_inventory['currency'] ?? ['cp' => 0, 'sp' => 0, 'gp' => 0, 'pp' => 0],
          'items' => $items,
        ];
      }
      catch (\Exception $e) {
