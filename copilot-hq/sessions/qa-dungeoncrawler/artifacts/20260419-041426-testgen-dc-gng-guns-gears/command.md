- Status: done
- Completed: 2026-04-19T04:17:22Z

# Test Plan Design: dc-gng-guns-gears
...
`qa-suites/products/dungeoncrawler/features/dc-gng-guns-gears.json`
...
1. **Create** `features/dc-gng-guns-gears/03-test-plan.md` — the test spec:
   - List every test case derived from the AC
   - For each: test description, which suite it will live in, expected HTTP status or behavior, roles covered
   - Flag any AC items that cannot be expressed as automation (note to PM)
2. **Create** `qa-suites/products/dungeoncrawler/features/dc-gng-guns-gears.json` from `templates/qa-feature-suite.json`:
   ...
   - Point `test_plan` at `features/dc-gng-guns-gears/03-test-plan.md`
   - Validate with:
     ```bash
     python3 scripts/qa-suite-validate.py --product dungeoncrawler --feature-id dc-gng-guns-gears
     ```
2. **Signal completion:**
    ```bash
    ./scripts/qa-pm-testgen-complete.sh dungeoncrawler dc-gng-guns-gears "<brief summary>"
   ```
