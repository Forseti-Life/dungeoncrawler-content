...
# using the HQ local LLM runtime (separate session id per agent).
...
# If the model file is present on disk, invoke llm/runner.py.
# If no on-disk model route is available, use host-local llama-server.
# External fallback backends are disabled for HQ release/SDLC execution.

_llm_resolve_model() {
  # Returns the model file path if a local model is assigned and present on disk.
  # Returns empty string to signal "use local-server".
  local agent="$1"
...
try:
    ...
    sys.exit(0)  # Any import/resolution error → fall back to local-server
PY
}
...
resolve_backend() {
    ...
    local-server)
      ...
      return 1
      ;;
    deepseek|copilot|bedrock)
      echo "ERROR: routing requested external backend '${ROUTED_ROUTE_ID}' for ${AGENT_ID}; policy requires local-only runtime." >&2
      return 1
      ;;
  esac
  ...
  case "$AGENTIC_BACKEND" in
    local-server)
      ...
      ;;
    *)
      echo "ERROR: invalid HQ_AGENTIC_BACKEND='$AGENTIC_BACKEND' (expected: local-server)" >&2
      return 1
      ;;
  esac
}
...
backend_retry_delay_seconds() {
  case "$GENAI_BACKEND" in
    local) echo "${LOCAL_LLM_RETRY_DELAY_SECONDS:-5}" ;;
...
if [ -n "$LOCAL_MODEL_FILE" ]; then
  ...
  # If local model returns empty, retry through local-server.
  if [ -z "$(printf '%s' "$response" | tr -d ' \t\r\n')" ]; then
