...
git_guard pull --ff-only --quiet 2>&1 | tail -2
PROCESSED=0
SKIPPED=0
...
if [[ $PROCESSED -gt 0 && "$DRY_RUN" -eq 0 ]]; then
  git_guard add "sessions/node-${THIS_NODE}/"
  git_guard commit -m "node-recv: processed ${PROCESSED} inbox message(s) on ${THIS_NODE}

Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>" 2>/dev/null || true
  git_guard push origin HEAD:main 2>/dev/null || true
fi
