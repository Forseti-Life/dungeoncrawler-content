...
source "$ROOT_DIR/scripts/lib/git-guard.sh"
...
# Workspace-scoped git wrapper with org-wide safety guard.
git_ws() { git_guard -C "$WORKSPACE_ROOT" "$@"; }

git_is_dirty() {
  [ -n "$(git_ws status --porcelain 2>/dev/null || true)" ]
}

current_branch() {
  git_ws rev-parse --abbrev-ref HEAD 2>/dev/null || echo main
}
...
ensure_branch() {
  ...
  if git_ws show-ref --verify --quiet "refs/heads/$target_branch"; then
    git_ws checkout "$target_branch" >/dev/null 2>&1
  else
    git_ws checkout -b "$target_branch" >/dev/null 2>&1
  fi
}
...
maybe_git_pull() {
  ...
  # Safety: never rebase/autostash. Fast-forward only.
  git_ws fetch --prune origin "$branch" >/dev/null 2>&1 || true
  git_ws merge --ff-only "origin/$branch" >/dev/null 2>&1 || true
}
...
maybe_commit_and_push() {
  ...
  if [ "$AUTO_COMMIT" = "1" ]; then
    ...
    git_ws add -A
    git_ws commit -m "dev-sync: ${topic} (${work_item:-no-work-item})" >/dev/null 2>&1 || true
  fi

  if [ "$AUTO_PUSH" = "1" ]; then
    local branch
    branch="$(current_branch)"
    git_ws push -u origin "$branch" >/dev/null 2>&1 || true
  fi
}
