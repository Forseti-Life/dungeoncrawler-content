...
PERSONA_SESSION_SCRIPT="${COPILOT_PERSONA_SESSION_SCRIPT:-$ROOT_DIR/scripts/persona-copilot-session.sh}"
...
detect_persona_seat() {
  local prompt="${1,,}"
  if [[ "$prompt" == *"take on the ceo persona"* ]] || \
     [[ "$prompt" == *"load the ceo"* ]] || \
     [[ "$prompt" == *"you are the ceo"* ]] || \
     [[ "$prompt" == *"resume ceo session"* ]]; then
    printf '%s' "ceo-copilot-2"
    return 0
  fi

  if [[ "$prompt" == *"take on the architect persona"* ]] || \
     [[ "$prompt" == *"load the architect"* ]] || \
     [[ "$prompt" == *"you are the architect"* ]] || \
     [[ "$prompt" == *"resume architect session"* ]]; then
    printf '%s' "architect-copilot"
    return 0
  fi
...
maybe_route_persona_session() {
  local prompt="$1"
  local seat=""
  seat="$(detect_persona_seat "$prompt" || true)"
  [ -n "$seat" ] || return 0

  local target_name="persona-${seat}"
  local resolved_id=""
  if [ -x "$PERSONA_SESSION_SCRIPT" ]; then
    resolved_id="$(bash "$PERSONA_SESSION_SCRIPT" "$seat" --session-id 2>/dev/null || true)"
  fi

  if [ -n "$resolved_id" ]; then
    refresh_session "$target_name"
    printf '%s\n' "$resolved_id" > "$SESSION_FILE"
    SESSION_ID="$resolved_id"
  elif [ "$SESSION_NAME" != "$target_name" ]; then
    refresh_session "$target_name"
  fi

  if [ -x "$PERSONA_SESSION_SCRIPT" ]; then
    bash "$PERSONA_SESSION_SCRIPT" "$seat" --set-session-id "$SESSION_ID" >/dev/null 2>&1 || true
  fi

  echo "[persona session: $seat -> $SESSION_ID]" >&2
}
...
while true; do
  ...
  maybe_route_persona_session "$line"
