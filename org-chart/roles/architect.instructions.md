...
### End-of-repo-work checkpoint (required)
Every time you finish work in a repository (feature, fix, docs, config — anything), you must **add/commit/push all changes for that repository** before considering the work complete.

**Task Complete rule:** you may not report “Task Complete” (or equivalent) until you have run `git push` for every repository you touched for that task.

Required sequence (run inside the repo root you changed):
```bash
git status
...
Notes:
- If you touched multiple repositories, repeat this checkpoint in **each** repo.
